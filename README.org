...
| 評価ラリー                  | 評価者↔PJMO往復（issue/reply/review/close）・AI自動指摘統合 |
| *評価区分切替*              | システム単位で DS-110（政府情報システム）/ DS-910（省システム・機微情報）を切替。AI自動指摘プロンプトが区分に応じて自動切替 |
| RBAC                        | admin/evaluator/pjmo/viewer の4ロール権限                 |
| AI自動レビュー              | Ollama（qwen2.5）による観点別評価：DS-910対象は機微情報・SBOM・国内立地・自律性を最優先／DS-110対象は予算妥当性・Cloud by Default・同等品条項を評価 |
| 評価確定                    | evaluator/admin による close 投稿・確定者・日時記録       |
| 監査ログ                    | 全操作（import/export・AI指摘の評価区分を含む）を audit_logs に記録 |
| Excel/PDFエクスポート       | openpyxl / reportlab による監査ログ出力                   |
| システム管理（CRUD）        | システムの追加・編集・区分切替・削除（事業0件時のみ）     |
| システムJSON入出力          | システム単位の JSON 出力／取込（DS-910区分も往復、同名重複チェック付き） |
| 事業JSON入出力              | 事業単位（事業設定＋ラリー履歴）の JSON エクスポート      |
...
#+begin_src sql :tangle db/init.sql
  ...
  -- ── 評価対象システム ──────────────────────────────────
  --   ds910_applicable:
  --     TRUE  … DS-910対象（省システム・機微情報システム）
  --             安全保障・外交・公共安全・経済安全保障に関わる情報を扱う
  --     FALSE … 政府情報システム一般（DS-110のみ準拠）
  ...
  -- 既存DBへの後方互換：ALTERで列を追加（init.sql 再実行時は無害）
  ALTER TABLE systems
  ...
  INSERT INTO systems (id, name, department, ds910_applicable) VALUES
      (1, '人事給与システム（次期）', '人事院',       FALSE),  -- DS-110評価
      (2, '行政DX基盤',               'デジタル庁',   FALSE),  -- DS-110評価
      (3, '安全保障情報連携基盤',     '内閣官房',     TRUE)    -- DS-910評価（機微情報）
  ON CONFLICT DO NOTHING;
  ...
  INSERT INTO projects (id, system_id, name, cost_category, amount_single, amount_deferred, evaluator_name) VALUES
      ...
      (6, 3, '令和8年度 セキュア環境整備', '整備費（投資的整備）',     220000, 120000, '鈴木部長')
  ON CONFLICT DO NOTHING;
...
** projects.py（*旧実装・廃止済み*）

旧スキーマ（=category= カラム・=review_logs= テーブル）を前提とした実装で、
現行のスキーマ（=systems= / =projects= / =review_threads= / =review_entries=）
とは整合しない。現行実装は =projects_router.py= に置き換え済み。

設計書 §3.2「事業管理」および §7「移行計画」との整合性確保のため、
=:tangle no= として生成対象から除外する（参考として残置）。
...
旧スキーマの =review_logs= テーブル（単一テーブルでバージョン管理）を前提とした実装。
現行は =review_threads= + =review_entries= の2テーブル構成に分離されており、
スレッド/投稿操作は =threads_router.py=、事業単位の入出力は =io_router.py= 配下に
再設計されている（設計書 §2.5–2.6, §3.3）。
...
class SystemCreate(BaseModel):
    ...
    ds910_applicable: Optional[bool] = False  # TRUE: DS-910対象（省システム・機微情報）


class SystemUpdate(BaseModel):
    name: Optional[str] = None
    department: Optional[str] = None
    ds910_applicable: Optional[bool] = None
...
def list_systems(db: Session = Depends(get_db),
    ...
    rows = db.execute(text("""
        SELECT s.id, s.name, s.department, s.ds910_applicable,
...
def create_system(data: SystemCreate, db: Session = Depends(get_db),
    ...
    row = db.execute(
        text("""INSERT INTO systems (name, department, ds910_applicable)
                VALUES (:n,:d,:ds) RETURNING id"""),
        {"n": data.name.strip(), "d": data.department,
         "ds": bool(data.ds910_applicable)}
    ).fetchone()
    sys_id = row[0]
    ...
    _audit(db, user.username, "create_system", sys_id,
           {"name": data.name, "ds910_applicable": bool(data.ds910_applicable)})
    db.commit()
...
def update_system(system_id: int, data: SystemUpdate,
    ...
    if data.ds910_applicable is not None:
        updates["ds910_applicable"] = bool(data.ds910_applicable)
    if updates:
...
async def auto_thread(data: AutoThreadRequest, db: Session = Depends(get_db),
    ...
    # 事業→システムを逆引きし、DS-910対象か判定する
    sys_row = db.execute(text("""
        SELECT s.id, s.name, s.department, s.ds910_applicable
        FROM projects p JOIN systems s ON s.id = p.system_id
        WHERE p.id = :pid
    """), {"pid": data.project_id}).fetchone()
    if not sys_row:
        raise HTTPException(404, "事業または所属システムが見つかりません")

    is_ds910 = bool(sys_row.ds910_applicable)

    # DS-910対象（省システム・機微情報システム）
    prompt_ds910 = """あなたは日本の政府IT調達のセキュリティアーキテクトです。
対象は *DS-910 対象システム*（安全保障等の機微な情報を取り扱う省システム）です。
以下の観点を最優先に、調達仕様書・見積書の妥当性を評価し、具体的な指摘事項を200字以内で記述してください。
...
副次的に DS-110（予算妥当性・調達透明性）も評価してください。
根拠条項を必ず明示し、「一式」表記などの根拠欠如は妥当性なしと判定してください。
「同等以上の機能・性能・互換性」条項の有無も確認してください。"""

    # 政府情報システム（DS-110のみ）
    prompt_ds110 = """あなたは日本の政府IT調達のデジタル統括アドバイザーです。
対象は *政府情報システム*（DS-110 準拠）です。
以下の観点から調達仕様書・見積書の妥当性を評価し、具体的な指摘事項を200字以内で記述してください。

【必須評価観点（DS-110 第3編第3章「予算及び執行」準拠）】
1. 予算マネジメント基礎: コスト分解、当初想定と実運用の乖離、調達方法の事前検討
2. 費用対効果・投資妥当性: システム機能範囲の把握、中長期投資計画、国庫債務負担行為
3. 資料準備・要求根拠の合理性: サービスレベルの必要十分性、実データ根拠、要求額の積算精査
4. 経費見積りの適正性: 積算内訳の明確化、LCC、システム単位積算、経費区分適用、複数見積比較、*Cloud by Default*
5. ガバナンス・承認プロセス: PMO資料提出、デジタル庁指摘対応、プロジェクト計画反映

根拠条項を必ず明示し、「一式」表記などの根拠欠如は妥当性なしと判定してください。
特定製品名を避け「同等以上の機能・性能・互換性を有する製品をもって代えることができる」旨の条項を求めてください。"""

    system_prompt = prompt_ds910 if is_ds910 else prompt_ds110

    # 対象システムの属性情報をユーザメッセージ冒頭に付与（AIが文脈を把握できるように）
    context_header = (
        f"【対象システム情報】\n"
        f"・システム名: {sys_row.name}\n"
        f"・担当部署: {sys_row.department or '未設定'}\n"
        f"・区分: {'DS-910対象（省システム・機微情報）' if is_ds910 else '政府情報システム（DS-110準拠）'}\n"
        f"\n【評価対象ドキュメント】\n"
    )
    user_content = context_header + data.content
    ...
    # スレッドタイトルに評価モードを自動付与（未指定時）
    title = data.title
    if not title:
        title = f"[AI自動指摘/{'DS-910' if is_ds910 else 'DS-110'}]"

    thread_no = _next_thread_no(db, data.project_id)
    t = db.execute(text("""
        INSERT INTO review_threads (project_id, thread_no, title, status)
        VALUES (:p, :tn, :ti, 'open') RETURNING id
    """), {"p": data.project_id, "tn": thread_no, "ti": title}).fetchone()
    thread_id = t[0]
    db.execute(text("""
    ...
    _audit(db, data.project_id, "ai", "auto_thread", thread_id,
           {"ds910_applicable": is_ds910, "system_id": sys_row.id})
    db.commit()
    return {"status": "ok", "id": thread_id, "thread_no": thread_no,
            "comment": comment,
            "mode": "DS-910" if is_ds910 else "DS-110"}
#+end_src
...
def _audit(db, project_id, user_name, action, target_table=None, target_id=None, detail=None):
    """監査ログ記録（DS-910 Ⅱ-11 監査体制）"""
    db.execute(
        text("""INSERT INTO audit_logs
                (project_id, user_name, action, target_table, target_id, detail)
                VALUES (:p,:u,:a,:tt,:ti, CAST(:d AS jsonb))"""),
        {"p": project_id, "u": user_name, "a": action,
         "tt": target_table, "ti": target_id,
         "d": json.dumps(detail or {}, ensure_ascii=False)}
    )
...
def _export_system(db, system_id: int) -> dict:
    sys_row = db.execute(
        text("SELECT id, name, department, ds910_applicable FROM systems WHERE id=:id"),
        {"id": system_id}
    ).fetchone()
    if not sys_row:
        raise HTTPException(404, "システムが見つかりません")
    ...
    return {
        ...
        "system": {
            "id": sys_row.id,
            "name": sys_row.name,
            "department": sys_row.department,
            "ds910_applicable": bool(sys_row.ds910_applicable),
        },
...
def export_system_json(system_id: int, db: Session = Depends(get_db),
    ...
    _audit(db, None, user.username, "export_system_json", "systems", system_id,
...
# ════════════════════════════════════════════════════════
#  事業単位JSONエクスポート（設計書 §3.4）
#  - システムJSONのサブセット。事業設定 + 配下全スレッドを含む
#  - 事業の移植・複製用途
# ════════════════════════════════════════════════════════

@router.get("/export/project/{project_id}/json")
def export_project_json(project_id: int, db: Session = Depends(get_db),
                        user: UserContext = Depends(get_current_user)):
    user.require("export")

    p = db.execute(text("""
        SELECT id, system_id, name, cost_category, is_system_wide,
               amount_single, amount_advance, amount_deferred,
               evaluator_name, estimate_text, requirements_text, basis_text
    ...
    _audit(db, project_id, user.username, "export_project_json", "projects", project_id,
           {"thread_count": len(result_threads)})
    db.commit()
    return result


# ════════════════════════════════════════════════════════
#  インポート前重複チェック（設計書 §6.2）
#  - システム名が既存と一致するかを事前に返却し、
#    フロントエンドで確認プロンプトを出せるようにする
# ════════════════════════════════════════════════════════

@router.post("/import/system/check")
def import_system_check(payload: dict, db: Session = Depends(get_db),
                        user: UserContext = Depends(get_current_user)):
    user.require("manage")
    ...
    name = (sys_data.get("name") or "").strip()
    if not name:
        return {"duplicate": False, "matched": []}

    rows = db.execute(
        text("SELECT id, name, department FROM systems WHERE name=:n ORDER BY id"),
        {"n": name}
    ).fetchall()

    return {
        "duplicate": len(rows) > 0,
        "matched": [dict(r._mapping) for r in rows],
        "summary": {
            "system_name": name,
...
def import_system_json(payload: dict, db: Session = Depends(get_db),
    ...
    sys_row = db.execute(
        text("""INSERT INTO systems (name, department, ds910_applicable)
                VALUES (:n,:d,:ds) RETURNING id"""),
        {"n": sys_data.get("name", "インポートシステム"),
         "d": sys_data.get("department"),
         "ds": bool(sys_data.get("ds910_applicable", False))}
    ).fetchone()
    system_id = sys_row[0]
    ...
    _audit(db, None, user.username, "import_system", "systems", system_id,
           {"system_name": sys_data.get("name"),
            "ds910_applicable": bool(sys_data.get("ds910_applicable", False)),
            "project_count": len(payload.get("projects", []))})
    db.commit()
...
def export_project_relay(project_id: int, db: Session = Depends(get_db),
    ...
    _audit(db, project_id, user.username, "export_project_relay", "projects", project_id,
           {"thread_count": len(result_threads)})
    db.commit()
    return result
...
def import_project_relay(project_id: int, payload: dict,
    ...
    _audit(db, project_id, user.username, "import_project_relay", "projects", project_id,
           {"mode": mode,
...
export interface SystemRow {
  ...
  name: string;
  department: string;
  ds910_applicable: boolean;  // TRUE: DS-910対象（省システム・機微情報）
  created_at: string;
...
function SystemsView({ headers, showToast }: { headers: () => Record<string, string>; showToast: (m: string) => void }) {
  ...
  const [newSysDS910, setNewSysDS910] = useState(false);  // DS-910対象フラグ
  ...
  const createSystem = () => {
    ...
    fetch(`${API}/systems/`, { method: "POST",
      ...
      body: JSON.stringify({
        name: newSysName,
        department: newSysDept,
        ds910_applicable: newSysDS910,
      })
    }).then(() => {
      setNewSysName(""); setNewSysDept(""); setNewSysDS910(false);
      setShowNewSys(false); loadSystems();
      showToast(newSysDS910 ? "DS-910対象システムを追加しました" : "システムを追加しました");
    });
  };

  // DS-910対象フラグの切り替え（既存システム）
  const toggleDS910 = (sys: SystemRow) => {
    const next = !sys.ds910_applicable;
    const label = next ? "DS-910対象" : "政府情報システム（DS-110）";
    if (!window.confirm(`「${sys.name}」の評価区分を「${label}」に変更しますか？\n以後のAI自動指摘のプロンプトが切り替わります。`)) return;
    fetch(`${API}/systems/${sys.id}`, { method: "PUT",
      headers: { "Content-Type": "application/json", ...headers() },
      body: JSON.stringify({ ds910_applicable: next })
    }).then(async r => {
      if (!r.ok) throw new Error("区分変更に失敗しました");
      loadSystems();
      showToast(`区分を「${label}」に変更しました`);
  ...
  // 設計書 §6.2: システム名が一致する場合は確認プロンプトを表示する
  const importSystem = async (file: File) => {
    ...
    // 1. 重複チェック（/import/system/check）
    try {
      const checkRes = await fetch(`${API}/import/system/check`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...headers() },
        body: JSON.stringify(payload),
      });
      if (!checkRes.ok) {
        const err = await checkRes.json().catch(() => ({ detail: "事前チェックに失敗しました" }));
        throw new Error(err.detail || "事前チェックに失敗しました");
      }
      const check = await checkRes.json();
      if (check.duplicate) {
        const names = (check.matched || []).map((s: any) => `#${s.id} ${s.name}`).join(", ");
        const ok = window.confirm(
          `同名のシステムが既に登録されています。\n` +
          `既存: ${names}\n\n` +
          `インポートは「新規採番」で実行されます（既存は上書きされません）。続行しますか？`
        );
        if (!ok) {
          showToast("インポートを中止しました");
          return;
        }
      }
    } catch (e) {
      showToast(`事前チェックエラー: ${(e as Error).message}`);
      return;
    }

    // 2. インポート実行
    try {
      const r = await fetch(`${API}/import/system`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...headers() },
        body: JSON.stringify(payload),
      });
      ...
      showToast(`インポートエラー: ${(e as Error).message}`);
    }
  };
    ...
    <div>
      ...
      {showNewSys && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.4)", zIndex: 1000,
          ...
          <div style={{ ...S.card, width: 440 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: 20, padding: 12,
                          background: newSysDS910 ? "#fef2f2" : "#f8fafc",
                          border: `1px solid ${newSysDS910 ? "#fecaca" : "#e2e8f0"}`,
                          borderRadius: 6 }}>
              <label style={{ display: "flex", alignItems: "flex-start", gap: 8, cursor: "pointer" }}>
                <input type="checkbox" checked={newSysDS910}
                  onChange={e => setNewSysDS910(e.target.checked)}
                  style={{ marginTop: 3 }} />
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: newSysDS910 ? "#dc2626" : "#0f172a" }}>
                    🔒 DS-910対象システムとして登録する
                  </div>
                  <div style={{ fontSize: 11, color: "#64748b", marginTop: 4, lineHeight: 1.5 }}>
                    安全保障等の機微な情報を取り扱う省システムの場合にチェック。<br />
                    AI自動指摘のプロンプトが <b>DS-910 観点</b>（機微情報・SBOM・国内立地・自律性等）に切り替わります。<br />
                    チェックなしの場合は <b>DS-110 観点</b>（予算妥当性・Cloud by Default等）で評価されます。
                  </div>
                </div>
              </label>
            </div>
      ...
      {systems.length === 0
        ...
        : systems.map(s => (
          <div key={s.id} style={{
            ...
            borderLeft: s.ds910_applicable ? "4px solid #dc2626" : "4px solid transparent",
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  {s.ds910_applicable ? (
                    <span title="DS-910対象システム（省システム・機微情報）"
                ...
                <div style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>{s.department || "担当部署未設定"}</div>
              </div>
              ...
              <button onClick={() => toggleDS910(s)} style={chipBtn(s.ds910_applicable ? "#1d4ed8" : "#dc2626")}
                title={s.ds910_applicable ? "DS-110（政府情報システム）に変更" : "DS-910対象（省システム）に変更"}>
                ⇄ 区分
              </button>
...
function RallyView({ initialSystemId, headers, showToast, currentRole }:
  ...
  const runAI = () => {
    ...
    fetch(`${API}/threads/auto`, { method: "POST",
      ...
      if (!r.ok) {
        const err = await r.json().catch(() => ({ detail: "AI指摘生成に失敗" }));
        throw new Error(err.detail || "AI指摘生成に失敗");
      }
      return r.json();
    }).then(res => {
      setAiText(""); reloadThreads();
      showToast(`AI指摘（${res?.mode || "DS-110"}）を追加しました`);
    }).catch(e => alert("エラー: " + e.message))
    .finally(() => setLoading(false));
  };

  const curThread = threads.find(t => t.id === selThread);
  const curSys = systems.find(s => s.id === selSys);
  const isDS910 = !!curSys?.ds910_applicable;
          ...
          <select value={selSys || ""} onChange={e => setSelSys(Number(e.target.value))}
            ...
            {systems.map(s => (
              <option key={s.id} value={s.id}>
                {s.ds910_applicable ? "🔒 " : ""}{s.name}
              </option>
            ))}
          </select>
          {/* 評価区分バッジ */}
          {curSys && (
            <div style={{ marginTop: 6, fontSize: 10, fontWeight: 700, padding: "3px 8px",
                          borderRadius: 10, textAlign: "center" as const,
                          background: isDS910 ? "#fee2e2" : "#eff6ff",
                          color:      isDS910 ? "#dc2626" : "#1d4ed8",
                          border: `1px solid ${isDS910 ? "#fecaca" : "#bfdbfe"}` }}>
              {isDS910 ? "🔒 DS-910 評価モード" : "DS-110 評価モード"}
            </div>
          )}
        </div>
      ...
      <div style={{ width: 280, flexShrink: 0, borderRight: "1px solid #e2e8f0",
        ...
        {canEvaluator && selPj && (
          <div style={{ padding: "10px 12px", borderTop: "1px solid #e2e8f0",
                        background: isDS910 ? "#fef2f2" : "transparent" }}>
            <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 6,
                          color: isDS910 ? "#dc2626" : "#6b21a8" }}>
              🤖 AI自動指摘（{isDS910 ? "DS-910観点" : "DS-110観点"}）
            </div>
            <textarea value={aiText} onChange={e => setAiText(e.target.value)}
              placeholder={isDS910
                ? "仕様書・見積書を貼り付け...（機微情報・SBOM・国内立地の観点で評価）"
                : "仕様書・見積書を貼り付け...（予算妥当性・Cloud by Defaultの観点で評価）"}
              rows={3}
            ...
            <button onClick={runAI} disabled={loading || !aiText.trim()}
              style={{ ...btn(isDS910 ? "#dc2626" : "#6b21a8", loading || !aiText.trim()),
                       marginTop: 6, width: "100%", fontSize: 11 }}>
              {loading ? "生成中..." : `AI指摘を生成（${isDS910 ? "DS-910" : "DS-110"}）`}
            </button>
          </div>
        )}
      </div>
...
| POST   | ~/import/system/check~                    | manage | インポート前の同名システム重複チェック（設計書§6.2）            |
| GET    | ~/export/project/{id}/json~               | export | 事業単位（事業設定＋配下スレッド）を JSON 出力（設計書§3.4）    |
...
** DS-910カラム追加時のマイグレーション（既存環境向け）

既存の =pgdata= ボリュームに対しては =init.sql= は再実行されないため、
v2.1 以降で追加された =systems.ds910_applicable= カラムは別途適用する必要がある。
=init.sql= には下記の =ALTER TABLE ... ADD COLUMN IF NOT EXISTS= を
同梱しているため、スクリプトを直接流し込めば既存DBにも安全に適用される。

#+begin_src shell :eval no
# 既存環境に DS-910 カラムだけ後から追加する
docker compose exec -T db psql -U budget -d budget <<SQL
ALTER TABLE systems
    ADD COLUMN IF NOT EXISTS ds910_applicable BOOLEAN NOT NULL DEFAULT FALSE;
SQL

# 既存システムを DS-910対象に切替える場合（例: system_id=3）
docker compose exec db psql -U budget -d budget \
  -c "UPDATE systems SET ds910_applicable = TRUE WHERE id = 3;"
#+end_src

既存システムを作り直しても構わない場合は、前項の =docker compose down -v=
で =pgdata= を削除する方が確実である。
